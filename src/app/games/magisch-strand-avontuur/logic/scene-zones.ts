...
export const getZoneCenter = (zone?: SceneZone): ScenePoint => {
  if (!zone) {
    return { x: 50, y: 50 };
  }
...
export const getZoneArea = (zone: SceneZone) => {
...
export const pointIsInsideZone = (point: ScenePoint, zone: SceneZone) => {
  const polygonPoints = parseSimplePolygonPath(zone.visualHintPath);
  if (polygonPoints.length >= 3) {
...
export const findSmallestZoneAtPoint = (zones: SceneZone[], point: ScenePoint) => {
  return zones
    .filter((zone) => pointIsInsideZone(point, zone))
    .sort((firstZone, secondZone) => getZoneArea(firstZone) - getZoneArea(secondZone))[0];
};
export const zoneSupportsConcept = (zone: SceneZone | undefined, concept: SpatialConcept) => {
  return Boolean(zone?.supportedConcepts.includes(concept));
};
export const selectedZoneMatchesTarget = (
