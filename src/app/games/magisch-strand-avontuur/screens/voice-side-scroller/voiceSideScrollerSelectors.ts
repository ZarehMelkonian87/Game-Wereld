...
export const getVisibleVoiceScrollerTargets = (targets: VoiceSideScrollerTarget[]) =>
  targets
    .filter(
      (target) =>
        !target.collected &&
...
export const getVoiceScrollerDifficultyProgress = (state: VoiceSideScrollerGameState) =>
  Math.round(state.distance % VOICE_SCROLLER_LEVEL_DISTANCE);

export const getVoiceScrollerDistanceMeters = (state: VoiceSideScrollerGameState) =>
