...
| **T-04** | P3 | 🎨🔧 | Modus **`zeg-en-bouw`** — concept vastgesteld → [Concept-Zeg-en-Bouw](Concept-Zeg-en-Bouw.md). **In uitvoering**. Subtaken: `T-04a` concept/docs ✅ · `T-04b` compound-parser ✅ · **`T-04c` scherm `SCR_MSA_ZEG_BOUW` + bouwkaarten (variant A)** — C1 datamodel+logica ✅, **C2a tap-bouwscherm ✅** (bouwkaart + voortgangsstippen + tik-om-te-plaatsen + soepele tip bij niet-passend + "Strand af!"-viering + Volgende strand; in browser end-to-end geverifieerd), C2b spraak/compound-invoer ⬜ · `T-04d` ronde-eindscherm + Strandschat-samenvatting ⬜ · `T-04e` variant B Vrij Bouwen (optioneel) ⬜. Modus toegevoegd aan moduskeuze + unlock-leerlijn **Kies→Zet→Bouw→Vlieg** (drempels 0/3/8/14; Zeg & Vlieg van 6→14 zodat Bouw ervóór komt, concept-besluit 4) | GAP-04 / 4.6 | 🟦 |
