...
| 1.41 | 2026-09-16 | Zareh Melkonian | **`T-48` (fix, hertest op apparaat):** Zeg & Vlieg-spraak stopte op mobiel na 2-3 objecten (`DT-02`). Oorzaak: 90 ms hersteltijd tussen matches → overlappende mic-sessies breken de mobiele spraak-API + herstel liep alleen op status `error` (stille `onend` op mobiel bleef "dood"). Fix in `useVoiceSideScrollerWordRecognition.ts`: hersteltijd → 350 ms en een robuuste **herstel-watchdog** die opnieuw gaat luisteren zolang de ronde loopt. Tsc/eslint/tests groen. |
...
| `GAP-05` | P2 | Spraak | Spraakconfig verschilt per scherm; oude GDD-getallen waren voorbeelden. Doc-deel ✅ via `T-09` (oude GDD weg, dossier correct); waarden-op-apparaat testen blijft `T-10` | Juiste waarden testen + doc bijwerken | [T-10](#12-takenlijst), [T-09](#12-takenlijst) |
...
| **T-10** | P2 | 🔍🎨 | Juiste spraakparameters per scherm bepalen en testen (stiltetijd, auto-stop, alternatieven). **Code-kant klaar** (waarden gecentraliseerd, gedocumenteerd via `T-09`); de *definitieve* waarden vereisen ijken op een echt apparaat met een kind → blijft open als apparaattaak | GAP-05 / 5.2 | ⬜ |
...
| **T-48** | **P1** | 🔧🔍 | **Zeg & Vlieg-spraak stopt op mobiel na 2-3 objecten** (gevonden bij `DT-02`). Oorzaak: te korte hersteltijd tussen matches (90 ms → overlappende mic-sessies breken de herkenning) + herstel alleen bij status `error` (stille `onend`/idle op mobiel → geen herstart). Fix: hersteltijd → 350 ms + robuuste **herstel-watchdog** die opnieuw gaat luisteren zolang de ronde loopt (`useVoiceSideScrollerWordRecognition.ts`). Tsc/eslint/tests groen; **hertest op apparaat nodig** (`DT-02`) | Testfeedback 2026-09-16 (`DT-02`) | 🟦 |
...
| `DT-02` | **Spraak — Zeg & Vlieg** | Start Zeg & Vlieg, benoem hardop de objecten die voorbij vliegen | Object wordt verzameld bij correct benoemen; **blijft werken na meerdere objecten** (niet stoppen na 2-3) | `T-18`, `T-48` | 🟦 Fix toegepast — hertesten |
...
| `DT-05` | **Audio-output** | Zet geluid aan; luister naar gesproken opdrachten/nazegzinnen en achtergrondmuziek | Audio is hoorbaar, duidelijk en op prettig volume; muziek volgt de `musicEnabled`-toggle | `T-18` | ⬜ |
| `DT-06` | **Rustige animaties (`T-14`)** | Instellingen → zet **"Rustige beweging"** aan; ga terug de modi in | De zweef-/sparkle-/parallax-animaties stoppen of zijn sterk gedempt | `T-14` | ⬜ |
| `DT-07` | **Focus-ring (`T-13`)** | Sluit een toetsenbord aan (of gebruik tab-toets); tab door knoppen in elke modus | Elk interactief element toont een duidelijke blauwe focus-ring; alle knoppen zijn ≥ 48×48 | `T-13` | ⬜ |
| `DT-08` | **Reset wist record (`T-11`)** | Haal een record in Zeg & Vlieg; ga naar Instellingen → **Reset voortgang** → bevestig; open Zeg & Vlieg opnieuw | Het persoonlijke record staat weer op 0 (record is echt gewist) | `T-11` | ⬜ |
| `DT-09` | **Mic geweigerd → toetsenbord** | Weiger de mic-toestemming; probeer een opdracht | Vriendelijke melding + het typen-alternatief werkt; geen doodlopende staat | `T-18` | ⬜ |
| `DT-10` | **Voortgangsscherm (`T-17`)** | Speel wat rondes; open het voortgangsscherm (`/progress`) | Categorie-uitsplitsing (taaldomeinen/ruimtebegrippen) + tempo-inzicht kloppen met wat je speelde | `T-17` | ⬜ |
